...
class SamplingManager:
    ...
    def execute_sampling_matlab(self, state_type: int = 2, noise_std: float = 0.02,
        ...
        self.matlab_bridge.send('tomo_coords', coords.reshape(1, -1))
        self.matlab_bridge.send('tomo_state', state.reshape(1, -1))
        self.matlab_bridge.send('tomo_wigner', wigner.reshape(1, -1))
        ...
        if state_type == 2:  # 相干态
            ...
            wigner_formula = f'''
                ...
                N2 = 2 * (1 + exp(-2*{alpha}^2));
                W = (W_plus + W_minus + interference * exp(-{alpha}^2)) / N2;
                W = W + {noise_std} * randn(size(W));
            '''
        else:
