...
def plot_fidelity_curves(tomo, save_dir):
    ...
    ax1.axhline(y=tomo.F_threshold, color='gray', linestyle=':', 
               linewidth=1.5, label=f'Target F = {tomo.F_threshold}')
    
    ax1.set_xlabel("Sampling Rate (%)", fontsize=12)
    ax1.set_ylabel("Fidelity", fontsize=12, color='b')
    ...
    ax1.set_title(f"Fidelity & Max Variance vs Sampling Rate\n{tomo.state_name}", 
                  fontsize=14, fontweight='bold')
